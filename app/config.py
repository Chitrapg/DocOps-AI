...
class Settings(BaseSettings):
    ...
    - Makes some infra credentials optional at import-time; call require_db() when DB access is needed.
    """

    # Load from .env by default; ignore extra keys to avoid failures when .env contains
    # other tool-specific variables with different names.
    model_config = SettingsConfigDict(
        env_file=".env",
        env_file_encoding="utf-8",
        extra="ignore",  # <--- ignore unknown/extra env keys instead of raising
    )
    ...
    # Make DB fields optional at import time; code should call require_db() before using DB
    PG_DB: Optional[str] = Field(None, env="PG_DB")
    ...
    # -----------------------
    # Helper validators / methods
    # -----------------------
    @validator("PG_PORT", pre=True)
    def _port_to_int(cls, v):
        # allow string port values in env
    ...
    def require_db(self) -> None:
        """
        Call this in code paths that require Postgres credentials.
        It raises a clear RuntimeError if DB credentials are not configured.
        """
